...
class ConsultaController extends Controller
{
    ...
    */
     ...
     *     tags={"Consulta"},
     *     summary="Listar todas as consultas",
     *     description="Retorna uma lista de todas as consultas registradas no sistema.",
     *     @OA\Response(
     *         response=200,
     *         description="Lista de consultas retornada com sucesso",
     *         @OA\JsonContent(
    ...
    {
        $consultas = Consulta::with(['medico.pessoa', 'paciente', 'tipo_consulta'])->get();
        return response()->json(['success' => true, 'data' => $consultas]);
    }

    /**
     * @OA\Get(
     *     path="/api/pacientes-hoje",
     *     tags={"Consulta"},
     *     summary="Listar pacientes com consultas marcadas para hoje",
     *     description="Retorna uma lista de pacientes que têm consultas agendadas para o dia atual.",
     *     @OA\Response(
     *         response=200,
     *         description="Lista de pacientes de hoje retornada com sucesso",
     *         @OA\JsonContent(
     *             type="object",
     *             @OA\Property(property="success", type="boolean", example=true),
     *             @OA\Property(property="data", type="array")
     ...
     * )
     */
    public function pacientesHoje(): JsonResponse
    {
        $hoje = now()->toDateString();
        
        $consultas = Consulta::where('data', $hoje)
            ->with(['medico.pessoa', 'paciente'])
            ...
            ->map(function ($consulta) {
                return [
                    ...
                    'paciente_iniciais' => $this->getInitiais($consulta->paciente->nome),
                    'medico_nome' => $consulta->medico->pessoa->nome,
                    'hora' => $consulta->hora_inicio?->format('H:i'),
                    'status' => $consulta->status,
                ];
            });

        return response()->json(['success' => true, 'data' => $consultas, 'total' => $consultas->count()]);
    }

    private function getInitiais(string $nome): string
    {
