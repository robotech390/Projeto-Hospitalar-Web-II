...
const PATIENTS = [
  { initials: 'MS', name: 'Maria Silva',  doctor: 'Dr. Carlos',  time: '08:30', status: 'consulting' },
  { initials: 'JS', name: 'João Santos',  doctor: 'Dra. Ana',    time: '09:00', status: 'waiting'    },
  { initials: 'AO', name: 'Ana Oliveira', doctor: 'Dr. Roberto', time: '09:30', status: 'waiting'    },
  { initials: 'PC', name: 'Pedro Costa',  doctor: 'Dra. Julia',  time: '10:00', status: 'checkin'    },
  { initials: 'LM', name: 'Laura Mendes', doctor: 'Dr. Carlos',  time: '10:30', status: 'scheduled'  },
]

const STATUS_LABELS = {
  consulting: { label: 'Em Consulta', class: 'bg-green-100 text-green-700' },
  waiting:    { label: 'Aguardando',  class: 'bg-orange-100 text-orange-700' },
  checkin:    { label: 'Check-in',    class: 'bg-teal-100 text-teal-700' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-0.5">Visão geral do hospital — {today}</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={WEEK_DATA} barSize={10} barGap={4}>
          ...
          {PATIENTS.map((p, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-50 transition-colors">
                <div className={`w-8 h-8 rounded-full ${AVATAR_COLORS[i % AVATAR_COLORS.length]} flex items-center justify-center shrink-0`}>
                  <span className="text-white text-xs font-semibold">{p.initials}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 truncate">{p.name}</p>
                  <p className="text-xs text-slate-400">{p.doctor}</p>
                </div>
                <p className="text-sm text-slate-500 font-medium shrink-0">{p.time}</p>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-md shrink-0 ${st.class}`}>
                  {st.label}
                </span>
              </div>
            )
